...
- **Claude Code's own reasoning** (planning, writing markup) is covered by your Claude subscription.
- **Invocation's on-site generation** — `invocation-generate-layout`, `invocation-refine-block`, `invocation-gather-site-context`, and the in-editor sidebar — runs on the **WordPress server** through the provider you configure in Connectors. That needs an **API key** (e.g. an Anthropic API key from console.anthropic.com), billed separately from your Claude subscription.

To use **only your Claude subscription** (no WordPress API key), drive it from Claude Code with the read + write tools and let Claude compose the markup itself:

`invocation-get-theme-context`, `invocation-list-patterns`, `invocation-search-media`, `invocation-search-internal-links` (context — no key) → Claude writes the blocks → `invocation-create-page` / `invocation-update-page` (persist — no key).

Only the server-side `generate-layout` / `refine-block` tools and the editor sidebar require a provider key.
